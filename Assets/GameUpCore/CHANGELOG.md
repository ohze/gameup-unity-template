...
### Added
- **Audio: preload clip không phát.** `AudioManager.PreloadAudio(identity | tên | AudioIdentityReference | list, onCompleted)` load sẵn asset Addressables và dữ liệu âm thanh (`LoadAudioData`) để `PlayAudio`/`PlayMusic` phát ngay trong frame được gọi thay vì chờ load. Thêm `IsAudioReady(...)`, `ReleaseAudio(identity)` (bỏ qua clip đang phát/đang load), cờ `AudioIdentity.preloadClips` để preload clip cùng `AudioDatabase`, và `PreloadIdentities(onCompleted)` để bootstrap chờ được. Callback luôn gọi đúng một lần kể cả khi lỗi. Template `CLAUDE.md`, skill `gameup-core-api` (dùng chung Claude + Cursor) và rule Cursor `gameup-core-usage.mdc` có thêm dòng API preload + mẫu code, để AI ở project khác dùng API này thay vì tự cache `AudioClip`; bấm **Cập nhật** trong `GameUp → Settings` để lấy bản mới.
