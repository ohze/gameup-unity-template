...
### Added
- **Package tuỳ chọn GameUp UI Builder (`com.ohze.gameup.uibuilder`).** `GameUpCore Installer` → mục *Tùy chọn* có card cài qua Git UPM (`?path=Assets/GameUpUIBuilder`); template `CLAUDE.md` §9 thêm dòng menu `GameUp → UI → UI Builder (Demo → Prefab)` — bấm **Cập nhật** trong `GameUp → Settings` để lấy.
- **Audio: preload clip không phát.** `AudioManager.PreloadAudio(identity | tên | AudioIdentityReference | list, onCompleted)` load sẵn asset Addressables và dữ liệu âm thanh (`LoadAudioData`) để `PlayAudio`/`PlayMusic` phát ngay trong frame được gọi thay vì chờ load. Thêm `IsAudioReady(...)`, cờ `AudioIdentity.preloadClips` để preload clip cùng `AudioDatabase`, và `PreloadIdentities(onCompleted)` để bootstrap chờ được. Callback luôn gọi đúng một lần kể cả khi lỗi.
- **Audio: unload để tối ưu RAM.** `UnloadAudioData(identity | tên)` xả dữ liệu âm thanh nhưng giữ asset; `ReleaseAudio(identity | tên)` xả data rồi nhả handle Addressables (xả data trước vì Addressables chỉ unload khi cả bundle hết người dùng) và gỡ clip khỏi `AudioSource` đã phát xong để asset thực sự được giải phóng; `ReleaseUnusedAudio()` nhả mọi clip không đang phát/đang load. Tuỳ chọn `releaseUnusedOnLowMemory` (mặc định bật) tự nhả khi `Application.lowMemory`.
  - Template `CLAUDE.md`, skill `gameup-core-api` (dùng chung Claude + Cursor) và rule Cursor `gameup-core-usage.mdc` có thêm dòng API preload/unload + mẫu code, để AI ở project khác dùng API này thay vì tự cache/unload `AudioClip`; bấm **Cập nhật** trong `GameUp → Settings` để lấy bản mới.
